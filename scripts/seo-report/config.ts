 ...
 * as a URL-prefix property ("https://barakaevents.com/") or a Domain property
 * ("sc-domain:barakaevents.com") — these are different identifiers to the API
 * even though they cover the same site. Default to the URL-prefix form (the
 * common case for a single-domain site with no subdomains to aggregate), but
 * let it be overridden via SEARCH_CONSOLE_SITE_URL without touching code, in
 * case the property was verified as a Domain property instead.
 */
export const SITE_URL = process.env.SEARCH_CONSOLE_SITE_URL || 'https://barakaevents.com/';

export const PRODUCTION_ORIGIN = 'https://barakaevents.com';
